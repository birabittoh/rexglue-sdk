...
u32 XamUserCheckPrivilege_entry(u32 user_index, u32 mask, mapped_u32 out_value) {
  ...
  // We advertise the user as signed in to Xbox Live (see
  // UserProfile::signin_state), so grant privileges rather than denying them.
  // The Leaderboards screen gates access on XamUserCheckPrivilege for the
  // presence/online privilege (mask 0xF9); denying it made the game report
  // "You are not signed in to Xbox Live." Granting keeps that gate consistent
  // with the signed-in-to-Live state.
  *out_value = 1;
  return X_ERROR_SUCCESS;
}
