...
static bool ProcessTitleUpdate(const std::filesystem::path& dir,
  ...
  // If update_data_root is already set, copy default.xexp from there.
  {
    std::string udr = REXCVAR_GET(update_data_root);
    if (!udr.empty()) {
      auto src = std::filesystem::path(udr) / "default.xexp";
      if (std::filesystem::is_regular_file(src)) {
        REXLOG_INFO("update_data_root has default.xexp, copying to {}", dir.string());
        std::filesystem::copy_file(src, dir / "default.xexp",
  ...
  // Prompt and extract.
  std::string msg =
      "This build of the game requires a title update to continue.\n\n"
