...
REXCVAR_DEFINE_UINT32(touch_user_index, 0, "Input/Touch",
                      "Controller slot the touch controls drive");
...
uint32_t TouchInputDriver::UserIndex() const {
  return REXCVAR_GET(touch_user_index);
}
...
X_RESULT TouchInputDriver::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  if (!REXCVAR_GET(touch_controls) || user_index != UserIndex()) {
    return X_ERROR_DEVICE_NOT_CONNECTED;
  }
