...
OverlayMenuDialog::OverlayMenuDialog(ImGuiDrawer* imgui_drawer, rex::Runtime* runtime)
  ...
  // Default trigger is Y, not RThumb: RThumb's stick is now used by the
  // gamepad UI controller (gamepad_ui.h) to move the active overlay around
  // the screen while in UI mode, so it can no longer double as a button
  // press without the two fighting over the same stick.
  rex::ui::RegisterBind(
      "bind_overlay_menu", "Y", "Toggle overlay menu", [this] { visible_ = !visible_; },
