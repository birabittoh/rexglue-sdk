...
void AchievementsOverlayDialog::OnDraw(ImGuiIO& io) {
  ...
  if (ImGui::Begin("Achievements##overlay", nullptr, ImGuiWindowFlags_NoCollapse)) {
    ...
    for (const auto& a : achievements) {
      ...
      if (is_secret) {
        const float row_content_height = std::max(style.icon_size, text_block_height);
      ...
      const float icon_draw_y = row_start_y + (row_end_y - row_start_y - style.icon_size) * 0.5f;
      const ImVec2 icon_draw_pos(icon_screen_pos.x, icon_draw_y);
      if (show_hidden) {
        DrawLockGlyph(draw_list, icon_draw_pos, style.icon_size,
                      ImGui::GetColorU32(UnlockedTitleColor()));
      } else if (icon) {
        ImVec4 tint = is_unlocked ? style.unlocked_icon_tint : style.locked_icon_tint;
        draw_list->AddImage(
            reinterpret_cast<ImTextureID>(icon), icon_draw_pos,
            ImVec2(icon_draw_pos.x + style.icon_size, icon_draw_pos.y + style.icon_size),
