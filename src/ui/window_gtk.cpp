...
void WaylandRegistryGlobalHandler(void* data, struct wl_registry* registry, uint32_t name,
  ...
  if (std::strcmp(interface, "wl_compositor") == 0) {
    ...
    reg_data->relative_pointer_manager =
...
void GTKWindow::LockWaylandPointer() {
  ...
  struct wl_pointer* wl_ptr =
      gdk_wayland_device_get_wl_pointer(gdk_pointer);
  if (!wl_ptr) {
    return;
  }
  GdkWindow* toplevel_gdk = gtk_widget_get_window(window_);
  struct wl_surface* wl_surf =
      gdk_wayland_window_get_wl_surface(toplevel_gdk);
  if (!wl_surf) {
    return;
  }

  wl_locked_pointer_ = zwp_pointer_constraints_v1_lock_pointer(
      wl_pointer_constraints_, wl_surf, wl_ptr,
  ...
  wl_relative_pointer_ = zwp_relative_pointer_manager_v1_get_relative_pointer(
      wl_relative_pointer_manager_, wl_ptr);
...
void GTKWindow::OnRelativePointerMotion(void* data,
  ...
  MouseEvent e(self, MouseEvent::Button::kNone,
               static_cast<int32_t>(self->wayland_synth_x_),
...
gboolean GTKWindow::WindowEventHandler(GdkEvent* event) {
  switch (event->type) {
    ...
    // compositor delivers pointer events to the toplevel GdkWindow. Translate
    // coordinates into drawing_area_ space and forward to HandleMouse so the
    // X11 path (which routes via DrawingAreaEventHandler) and the Wayland path
    // both arrive at the same handler.
    case GDK_MOTION_NOTIFY: {
      auto* ev = reinterpret_cast<GdkEventMotion*>(event);
      gint dx = 0, dy = 0;
      gtk_widget_translate_coordinates(window_, drawing_area_,
                                       static_cast<gint>(ev->x), static_cast<gint>(ev->y),
                                       &dx, &dy);
      ev->x = dx;
      ev->y = dy;
      WindowDestructionReceiver destruction_receiver(this);
