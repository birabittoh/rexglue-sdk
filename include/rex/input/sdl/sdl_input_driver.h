...
class SDLInputDriver final : public InputDriver, public rex::ui::WindowListener {
 ...
 private:
  ...
  // Latest requested rumble per device; only the newest value matters. Keyed
  // by DeviceId rather than a vector index, since controllers_ reindexes on
  // removal but DeviceId stays stable for the lifetime of a connection.
  std::unordered_map<DeviceId, RumbleRequest> rumble_pending_;
  ...
  // pointer the guest thread might be about to free.
  std::unordered_map<DeviceId, SDL_JoystickID> hid_instance_;
